...
```
### カメラトラッキング
`config.yaml` に `track_body` を設定すると、カメラが自動的にロボットを追従します：
```yaml
track_body: "torso_link" # カメラが追従するボディ名。空にすると無効。
```
`mjCAMERA_TRACKING` モードで指定したボディをビューポートの中心に保持します。Go2/B2 では `"base_link"` を使用してください。
